...
use mouse::{
    definitions::UnsafeSyncRefCell,
    error::Result,
    ext::{PathExt, SizeOfVal, StaticSize, Transmutations, Uninitialized, UninitializedCollection},
    helpers::{
        object_as_slice, object_as_slice_mut, open_rwc_all, open_rwc_async, ptr_as_slice,
        ptr_as_slice_mut,
    },
    num::{rust_decimal::prelude::ToPrimitive, NumExt},
    prelude::*,
    time::{IntoDateTime, Timestamp},
};
use nebuchadnezzar::core::{
    paginators::WhileSuperPaginator, requests::TradesGetRequest, Credentials,
};
use tokio::{
    fs::File,
    io,
    io::{AsyncReadExt, AsyncSeek, AsyncSeekExt, AsyncWrite, AsyncWriteExt, BufReader, BufWriter},
};
...
pub async fn load_hlcv(
    ...
    if path.exists_async().await {
        ...
        println!(
            "saved start: {}, saved end: {}",
            header.start_ts, header.end_timestamp_s,
        );
        let mut file_writer;
...
async fn read_hlcv(
    ...
    use std::io::Seek;
    debug!("file size {:#?}", file.seek(SeekFrom::End(0)).unwrap());
    debug!("mapping {:#?}", src_len * Hlcv::size());
    let mmap = unsafe {
        MmapOptions::new()
            .offset(offset)
